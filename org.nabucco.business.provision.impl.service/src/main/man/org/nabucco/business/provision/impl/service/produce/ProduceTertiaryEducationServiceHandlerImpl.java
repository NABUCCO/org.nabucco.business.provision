...
/**
 * 
 * ProduceTeritaryEducationServiceHandlerImpl
 * 
 * @author Dominic Trumpfheller, PRODYNA AG
 */
public class ProduceTertiaryEducationServiceHandlerImpl extends ProduceTertiaryEducationServiceHandler {

    private static final long serialVersionUID = 1L;

    @Override
    protected TertiaryEducationListMsg produceTertiaryEducation(TertiaryEducationProduceRq rq) throws ProduceException {

        TertiaryEducationListMsg response = new TertiaryEducationListMsg();

        for (int i = 0; i < rq.getAmount().getValue(); i++) {
            TertiaryEducation datatype = new TertiaryEducation();
            datatype.setDatatypeState(DatatypeState.INITIALIZED);
            datatype.setOwner(getContext().getSubject().getOwner());

            response.getTertiaryEducationList().add(datatype);
        }
