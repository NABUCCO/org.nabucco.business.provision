...
/**
 * ProduceWorkExperienceServiceHandlerImpl
 * 
 * @author Dominic Trumpfheller, PRODYNA AG
 */
public class ProduceWorkExperienceServiceHandlerImpl extends ProduceWorkExperienceServiceHandler {

    private static final long serialVersionUID = 1L;

    @Override
    protected WorkExperienceListMsg produceWorkExperience(WorkExperienceProduceRq rq) throws ProduceException {

        WorkExperienceListMsg response = new WorkExperienceListMsg();

        for (int i = 0; i < rq.getAmount().getValue(); i++) {
            WorkExperience datatype = new WorkExperience();
            datatype.setDatatypeState(DatatypeState.INITIALIZED);
            datatype.setOwner(getContext().getSubject().getOwner());

            response.getWorkExperienceList().add(datatype);
        }
