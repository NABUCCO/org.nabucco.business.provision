...
/**
 * 
 * ProduceJobEducationServiceHandlerImpl
 * 
 * @author Dominic Trumpfheller, PRODYNA AG
 */
public class ProduceJobEducationServiceHandlerImpl extends ProduceJobEducationServiceHandler {

    private static final long serialVersionUID = 1L;

    @Override
    protected JobEducationListMsg produceJobEducation(JobEducationProduceRq rq) throws ProduceException {

        JobEducationListMsg response = new JobEducationListMsg();

        for (int i = 0; i < rq.getAmount().getValue(); i++) {
            JobEducation datatype = new JobEducation();
            datatype.setDatatypeState(DatatypeState.INITIALIZED);
            datatype.setOwner(getContext().getSubject().getOwner());

            response.getJobEducationList().add(datatype);
        }
