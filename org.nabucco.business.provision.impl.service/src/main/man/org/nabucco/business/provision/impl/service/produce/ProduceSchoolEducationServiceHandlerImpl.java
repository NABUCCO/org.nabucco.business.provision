...
/**
 * ProduceSchoolEducationServiceHandlerImpl
 * 
 * @author Dominic Trumpfheller, PRODYNA AG
 */
public class ProduceSchoolEducationServiceHandlerImpl extends ProduceSchoolEducationServiceHandler {

    private static final long serialVersionUID = 1L;

    @Override
    protected SchoolEducationListMsg produceSchoolEducation(SchoolEducationProduceRq rq) throws ProduceException {

        SchoolEducationListMsg response = new SchoolEducationListMsg();

        for (int i = 0; i < rq.getAmount().getValue(); i++) {
            SchoolEducation datatype = new SchoolEducation();
            datatype.setDatatypeState(DatatypeState.INITIALIZED);
            datatype.setOwner(getContext().getSubject().getOwner());

            response.getSchoolEducationList().add(datatype);
        }
