...
/**
 * 
 * ProduceQualificationServiceHandlerImpl
 * 
 * @author Dominic Trumpfheller, PRODYNA AG
 */
public class ProduceQualificationServiceHandlerImpl extends ProduceQualificationServiceHandler {

    private static final long serialVersionUID = 1L;

    @Override
    protected QualificationListMsg produceQualification(QualificationProduceRq rq) throws ProduceException {

        QualificationListMsg response = new QualificationListMsg();

        for (int i = 0; i < rq.getAmount().getValue(); i++) {
            Qualification datatype = new Qualification();
            datatype.setDatatypeState(DatatypeState.INITIALIZED);
            datatype.setOwner(getContext().getSubject().getOwner());

            response.getQualificationList().add(datatype);
        }
