...
/**
 * 
 * ProduceSkillServiceHandlerImpl
 * 
 * @author Dominic Trumpfheller, PRODYNA AG
 */
public class ProduceSkillServiceHandlerImpl extends ProduceSkillServiceHandler {

    private static final long serialVersionUID = 1L;

    @Override
    protected SkillListMsg produceSkill(SkillProduceRq rq) throws ProduceException {

        SkillListMsg response = new SkillListMsg();

        for (int i = 0; i < rq.getAmount().getValue(); i++) {
            Skill datatype = new Skill();
            datatype.setDatatypeState(DatatypeState.INITIALIZED);
            datatype.setOwner(getContext().getSubject().getOwner());

            response.getSkillList().add(datatype);
        }
